...
func natsMsgSubGroup(cli *Client, subFunc []NatsSubFunc) {
	fmt.Println("add the subject")
	for i := 0; i < len(subFunc); i++ {
		err := cli.Sub(subFunc[i].subject, subFunc[i].execFunc)
		if err != nil {
...
func NatsMsgSub(subject string, execFunc func(*nats.Msg)) {
	fmt.Println("add the subject and callback func")
	err := natsClient.Sub(subject, execFunc)
	if err != nil {
		fmt.Println("nats msg sub failed ", err)
	}
}
